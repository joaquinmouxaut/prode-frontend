...
> **Última sesión:** Martes 23 de junio de 2026 — **UI: agrupamiento de partidos en 3 niveles anidados (Fase → Jornada → Día) en fixture/participante/admin + fix indicador "Vos" del ranking. Deployado a prod (push a master → Vercel).**
...
| **Lun 22-jun** | **Deploy mata-mata + prueba local** | Probada la lógica knockout en local creando partidos a mano (`POST /matches` con JWT + `PATCH` para simular kickoff; guía `docs/knockout-local-testing.md`) — puntajes OK. **Rollout a prod:** branch Neon `pre-knockout-20260622` + `pg_dump -Fc` verificado (requirió `pg_dump` 18 porque Neon ya corre **PG 18.4**; anotado en `db-backup-restore.md`); `prisma migrate deploy` aplicó `20260619110000_knockout_advancement` (endpoint directo) → "schema up to date"; commit+push back/front → Render + Vercel redeploy. | `prode/session-latest` |

| **Mar 23-jun** | **UI — agrupamiento anidado de partidos + fix "Vos"** | Agrupamiento de partidos pasó de 1 nivel (día) a **3 niveles con contención visual** (caja dentro de caja): **Fase** (Fase de grupos / Mata-mata) ⊃ **Jornada** (Jornada 1/2/3, Dieciseisavos, Octavos, Cuartos, Semifinal, Tercer puesto, Final) ⊃ **Día** ⊃ partidos. Aplicado a las **3 pantallas**: `/fixtures`, `/participantes/:id`, `/admin`. Helpers nuevos en `core/models/match-phase.ts` (`MatchStage`, `STAGE_LABELS`, `phaseStage`, `phaseOrder`, `formatJornadaLabel`) + util compartido nuevo `core/utils/fixture-grouping.ts` (`buildFixtureTree<T>()` genérico + `resolveActivePath()`). Cada pantalla con 3 sets de expansión (stage/jornada/día) + **auto-despliegue y auto-scroll al día actual** preservados. **Mobile:** niveles fase/jornada con padding mínimo (`p-1.5`→`sm:p-2.5`) para no estrechar las tarjetas de partido. **Ranking:** indicador "Vos" reestructurado a flex (nombre `truncate` + chip `shrink-0`) para que no quede apretado. `ng lint` OK, `ng build` OK (`ng test` no corrió: vitest watch bloquea el shell del entorno; `match-phase.spec` no afectado por cambios aditivos). Deploy a prod vía push a master (Vercel). | `prode/session-latest` |

**Carry-over:**
- **Pendiente verificación visual** del nuevo agrupamiento en prod (profundidad de colores/padding por nivel) — ajustar si hace falta.
- Correr `ng test` localmente para confirmar verde completo (no pudo ejecutarse en el entorno del agente por el watch mode).
- **Mata-mata:** ✅ deployado. Pendiente solo cuando la API publique los cruces: `POST /admin/fixture/import` (reasigna fases, incl. `ROUND_OF_32`) + smoke de un knockout real (por penales).
- Verificar logo/favicon en prod. Opcionales: quitar redundancia del '93' en header; `brand-mark` `lg`=512 upscalea el source de 256px.
